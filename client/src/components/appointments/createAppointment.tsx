...
export default function CreateAppointmentDialog({
  ...
  const handleSave = async () => {
    if (!selectedDate || !timeSlot || !patientId) return;

    setIsLoading(true);

    // ✅ Combine date & time
    const appointmentDateTime = new Date(selectedDate);
    const [hourPart, minutePartWithAmPm] = timeSlot.split(":");
    const minutePart = minutePartWithAmPm.replace(/\D/g, ""); // Remove AM/PM
    ...
    appointmentDateTime.setHours(hour, minutes);

    const payload: CreateAppointmentPayload = {
      patientId,
      dateTime: appointmentDateTime.toISOString(),
      description: "", // Optional, can be filled later
      name: "", // Will be auto-filled from patientId backend
      phoneNumber: "",
      address: "",
      age: 0,
      gender: "",
